...
P_vec = 5:5:65; % Pilot vector length
...
% Result vector
NMSE_oracle_avg = zeros(length(P_vec), 1); % NMSE, Oracle LS result vector
NMSE_ls_avg = zeros(length(P_vec), 1); % NMSE, LS result vector
NMSE_asomp_avg = zeros(length(P_vec), 1); % NMSE, AOMP result vector
NMSE_psomp_avg = zeros(length(P_vec), 1); % NMSE, SOMP result vector
NMSE_asigw_avg = zeros(length(P_vec), 1); % NMSE, A-SIGW result vector
NMSE_psigw_avg = zeros(length(P_vec), 1); % NMSE, P-SIGW result vector
NMSE_sbl_near_avg = zeros(length(P_vec), 1); % NMSE, Polar SBL result vector
...
for itr = 1:iteration
    ...
    %-------------------
    % For each Pilot symbol length vector index
    for P_index=1:length(P_vec)
        ...
        NMSE_oracle_avg(P_index) = NMSE_oracle_avg(P_index) + mse_oracle/normalize;
    
        %-------------------
        % LS results
        H_ls = pinv(A_bar) * Y_bar;
        mse_ls = norm(H-H_ls,"fro")^2;
        NMSE_ls_avg(P_index) = NMSE_ls_avg(P_index) + mse_ls/normalize;
        ...
        NMSE_asomp_avg(P_index) = NMSE_asomp_avg(P_index) + mse_asomp/normalize;                
        ...
        NMSE_psomp_avg(P_index) = NMSE_psomp_avg(P_index) + mse_psomp/normalize;
        ...
        NMSE_asigw_avg(P_index) = NMSE_asigw_avg(P_index) + mse_asigw/normalize;
        ...
        NMSE_psigw_avg(P_index) = NMSE_psigw_avg(P_index) + mse_psigw/normalize;
        ...
        NMSE_sbl_near_avg(P_index) = NMSE_sbl_near_avg(P_index) + mse_sbl_near/normalize;

        % Print in Command window...    
        disp(strcat("Iteration complete. (#: ", string(itr), ", Pilot length: ", string(P_vec(P_index)), ")"))        
    end
end
...
xlabel('SNR (dB)');
ylabel('NMSE (dB)');
xlim(P_vec([1 length(P_vec)]));
%ylim([-25 0]);
grid on;
hold on;

plot(P_vec, NMSE_oracle_avg, "black--", 'DisplayName', 'Oracle-LS', 'MarkerSize', 3);
% plot(P_vec, NMSE_ls_avg, "black<-", 'DisplayName', 'LS', 'MarkerSize', 3);
plot(P_vec, NMSE_asomp_avg, "cyan>--", 'DisplayName', 'Angular domain SOMP', 'MarkerSize', 3);
plot(P_vec, NMSE_psomp_avg, "cyan<-", 'DisplayName', 'Proposed P-SOMP', 'MarkerSize', 3);
plot(P_vec, NMSE_asigw_avg, "blue>--", 'DisplayName', 'Angular SIGW', 'MarkerSize', 3);
plot(P_vec, NMSE_psigw_avg, "blue<-", 'DisplayName', 'Proposed P-SIGW', 'MarkerSize', 3);
plot(P_vec, NMSE_sbl_near_avg, "redsquare-", 'DisplayName', 'Near field SBL', 'MarkerSize', 3);
