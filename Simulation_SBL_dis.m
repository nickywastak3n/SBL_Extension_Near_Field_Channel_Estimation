...
dis_num = 5;
r_dis = linspace(60,120,dis_num); % Uniform distribution of path distance

% Result vector
NMSE_oracle_avg = zeros(length(r_dis), 1); % NMSE, Oracle least squares result vector
NMSE_ls_avg = zeros(length(r_dis), 1); % NMSE, least squares result vector
NMSE_asomp_avg = zeros(length(r_dis), 1); % NMSE, AOMP result vector
NMSE_psomp_avg = zeros(length(r_dis), 1); % NMSE, SOMP result vector
NMSE_asigw_avg = zeros(length(r_dis), 1); % NMSE, A-SIGW result vector
NMSE_psigw_avg = zeros(length(r_dis), 1); % NMSE, P-SIGW result vector
NMSE_sbl_near_avg = zeros(length(r_dis), 1); % NMSE, Polar SBL result vector
...
for itr = 1:iteration
    ...
    %-------------------
    % For each distance index...
    for r_index=1:length(r_dis)
        %-------------------
        % Random generation for each distance index...

        % Channel parameters, Channel matrix
        r_bs = r_dis(r_index) * ones(1, L);
        ...
        NMSE_oracle_avg(r_index) = NMSE_oracle_avg(r_index) + mse_oracle/normalize;

        %-------------------
        % LS results
        H_ls = pinv(A_bar) * Y_bar;
        mse_ls = norm(H-H_ls,"fro")^2;
        NMSE_ls_avg(r_index) = NMSE_ls_avg(r_index) + mse_ls/normalize;
        ...
        NMSE_asomp_avg(r_index) = NMSE_asomp_avg(r_index) + mse_asomp/normalize;
        ...
        NMSE_psomp_avg(r_index) = NMSE_psomp_avg(r_index) + mse_psomp/normalize;
        ...
        NMSE_asigw_avg(r_index) = NMSE_asigw_avg(r_index) + mse_asigw/normalize;
        ...
        NMSE_psigw_avg(r_index) = NMSE_psigw_avg(r_index) + mse_psigw/normalize;
        ...
        NMSE_sbl_near_avg(r_index) = NMSE_sbl_near_avg(r_index) + mse_sbl_near/normalize;

        % Print in Command window...    
        disp(strcat("Iteration complete. (#: ", string(itr), ", Dis: ", string(r_dis(r_index)), ")"))
    end
end
...
xlabel('Distance (m)');
ylabel('NMSE (dB)');
xlim(r_dis([1 length(r_dis)]));
%ylim([-25 0]);
grid on;
hold on;

x1 = xline(Z_rayleigh, '-.', 'Rayleigh distance');
x1.LabelVerticalAlignment = 'middle';
x1.LabelHorizontalAlignment = 'center';
legend('');

plot(r_dis, NMSE_oracle_avg, "black--", 'DisplayName', 'Oracle-LS', 'MarkerSize', 3);
plot(r_dis, NMSE_ls_avg, "black<-", 'DisplayName', 'LS', 'MarkerSize', 3);
plot(r_dis, NMSE_asomp_avg, "cyan>--", 'DisplayName', 'Angular domain SOMP', 'MarkerSize', 3);
plot(r_dis, NMSE_psomp_avg, "cyan<-", 'DisplayName', 'Proposed P-SOMP', 'MarkerSize', 3);
plot(r_dis, NMSE_asigw_avg, "blue>--", 'DisplayName', 'Angular SIGW', 'MarkerSize', 3);
plot(r_dis, NMSE_psigw_avg, "blue<-", 'DisplayName', 'Proposed P-SIGW', 'MarkerSize', 3);
plot(r_dis, NMSE_sbl_near_avg, "redsquare-", 'DisplayName', 'Near field SBL', 'MarkerSize', 3);
